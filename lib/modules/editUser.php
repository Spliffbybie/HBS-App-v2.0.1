...
$id = $_GET['id']; $ut = $_GET['ut'];
	...
	if ((isset($_GET['ut'])) && ($ut == "Limited")){

 mysql_select_db($database_db_link, $db_link);

 $sql_acc = "SELECT First_name, Last_name, Email, login_alias FROM sys_users WHERE User_id = '$id' LIMIT 1";
...
$editFormAction = $_SERVER['PHP_SELF'];
  ...
  $updateSQL = sprintf("UPDATE sys_users SET First_name=%s, Last_name=%s, Email=%s, login_alias=%s, User_type=%s WHERE User_id = '$id' LIMIT 1",
...
//validate current password
  ...
  $updateSQL = sprintf("UPDATE sys_users SET login_password=md5(%s) WHERE User_id = '$id' LIMIT 1",
